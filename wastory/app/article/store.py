...
class ArticleStore :
    ...
    async def get_article_information_by_id(self, article_id : int) -> ArticleInformationResponse:
        ...
        stmt = (
            ...
            .filter(Article.id == article_id,Article.draft == False)
    ...
    async def get_articles_by_ids(self, article_ids: list[int]) -> Sequence[Article]:
        article_list_query = select(Article).where(Article.id.in_(article_ids),Article.draft==False)
    ...
    async def get_most_viewed_in_hometopic(
        ...
        stmt = (
            ...
            .join(Like, Like.article_id == Article.id, isouter=True)  # 좋아요 조인
            .join(Comment, Comment.article_id == Article.id, isouter=True)  # 댓글 조인
            .filter(Article.hometopic_id.in_(hometopic_id_list),Article.draft==False)  # 홈토픽 ID 필터링
            .group_by(Article.id, Blog.blog_name, Blog.main_image_url)
            .order_by(sort_column)  # 조회수 기준 정렬
            .offset(offset)  # 페이지 오프셋 적용
            .limit(per_page)  # 페이지당 제한
    ...
    async def get_articles_in_blog(self, blog_id: int, page: int, per_page: int) -> PaginatedArticleListResponse:
        ...
        stmt = (
            ...
            .join(Like, Like.article_id == Article.id, isouter=True)  # likes 조인
            .join(Comment, Comment.article_id == Article.id, isouter=True)  # comments 조인
            .filter(Article.blog_id == blog_id,Article.draft==False)
            .group_by(Article.id, Blog.blog_name, Blog.main_image_url)
        ...
        # 전체 개수 계산
        total_count_stmt = select(func.count(Article.id)).filter(Article.blog_id == blog_id,Article.draft==False)
        total_count = await SESSION.scalar(total_count_stmt)
    ...
    async def get_top_articles_in_blog(
        ...
        stmt = (
            ...
            .join(Like, Like.article_id == Article.id, isouter=True)  # likes 조인
            .join(Comment, Comment.article_id == Article.id, isouter=True)  # comments 조인
            .filter(Article.blog_id == blog_id,Article.draft==False)  # 특정 블로그에서 필터링
            .group_by(Article.id, Blog.blog_name, Blog.main_image_url)
    ...
    async def get_articles_by_words_and_blog_id(
        ...
        # 전체 개수 계산
        total_count_stmt = select(func.count(Article.id)).where(
            and_(*search_conditions, Article.draft == False)  # 쉼표 수정
            )
        total_count = await SESSION.scalar(total_count_stmt)
    ...
    async def get_articles_of_subscriptions(
        ...
        stmt = (
            ...
            .join(Like, Like.article_id == Article.id, isouter=True)  # likes 조인
            .join(Comment, Comment.article_id == Article.id, isouter=True)  # comments 조인
            .filter(Article.blog_id.in_(subscribed_ids),Article.draft==False)  # 구독한 블로그의 Article만 필터
            .group_by(Article.id, Blog.blog_name, Blog.main_image_url)
        ...
        # 전체 개수 계산
        total_count_stmt = select(func.count(Article.id)).filter(Article.blog_id.in_(subscribed_ids),Article.draft==False)
        total_count = await SESSION.scalar(total_count_stmt)
    ...
    @transactional
    async def get_drafts_in_blog(self, blog_id: int, page: int, per_page: int) -> PaginatedArticleListResponse:
        offset_val = (page - 1) * per_page
        ...
        stmt = (
            ...
            .join(Like, Like.article_id == Article.id, isouter=True)  # likes 조인
            .join(Comment, Comment.article_id == Article.id, isouter=True)  # comments 조인
            .filter(Article.blog_id == blog_id,Article.draft==True)
            .group_by(Article.id, Blog.blog_name, Blog.main_image_url)
            .order_by(Article.created_at.desc())
            .offset(offset_val)
            .limit(per_page)
